...
const OrganizationMembersList: React.FC<OrganizationMembersListProps> = ({
  ...
  const handleMemberTableUpdate = async () => {
    ...
    const userData = settings?.members.map(
      (member: { personId: any; email: any; permissions: any }) => ({
        key: member.personId,
        email: member.email,
  ...
  const handleAddMember = async () => {
    if (organizationId) {
      ...
      const response = await fetch(
        `${
          import.meta.env.VITE_APP_API_URL
        }/api/organization/members?organizationId=${organizationId}&email=${memberEmail}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (organizationId) {
        if (memberData?.length == 1) {
          ...
          await fetch(
            `${
              import.meta.env.VITE_APP_API_URL
            }/api/organization/members?organizationId=${organizationId}&personId=${personId}`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
    ...
    <>
      ...
      >
        ...
        <Checkbox.Group
          ...
          defaultValue={memberPermissions}
        >
          <Checkbox value="READ" className="settings-checkbox">
            READ
          </Checkbox>
          <Checkbox value="MANAGE_DATA" className="settings-checkbox">
            MANAGE_DATA
          </Checkbox>
          <Checkbox value="CREATE_TOPICS" className="settings-checkbox">
            CREATE_TOPICS
          </Checkbox>
          <Checkbox value="UPLOAD_DATA" className="settings-checkbox">
            UPLOAD_DATA
          </Checkbox>
          <Checkbox value="MANAGE_ORGANIZATION" className="settings-checkbox">
            MANAGE_ORGANIZATION
          </Checkbox>
          <Checkbox value="MANAGE_INTEGRATIONS" className="settings-checkbox">
            MANAGE_INTEGRATIONS
          </Checkbox>
        </Checkbox.Group>
      </Modal>
