...
const QuestionOverrideForm: React.FC<QuestionOverrideFormProps> = ({
  organizationId,
  setAlertMessage,
}) => {
  const [loading, setLoading] = useState(false);
  const [questionsOverride, setQuestionsOverride] =
    useState<QuestionsOverrideResponse>({
      questions: [],
    });
  ...
  const addQuestionToOverride = async (values: {
    question: string;
    answer: string;
  }) => {
    setLoading(true);
    ...
    const response = await fetch(
      ...
      }/api/organization/question-override?organizationId=${organizationId}`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <Card
      title={
        <span>
          Question override{" "}
          <BiSolidHelpCircle title="Overridden questions will always return the same response" />
        </span>
      }
    >
      <Loading />
      ...
      <Table
        dataSource={questionsOverride?.questions}
        columns={questionOverrideColumns}
      />
    </Card>
  );
};
