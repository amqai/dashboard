...
const PdfTable: React.FC<PdfTableProps> = ({ organizationId, topicId }) => {
  ...
  const loadTableData = async () => {
    ...
    }/api/pdf?organizationId=${organizationId}&topicId=${topicId}`;
  ...
  const pollStatus = async (pdfId: string) => {
    ...
    while (status === "ACTIVE_INGESTING") {
      const response = await fetch(
        `${
          import.meta.env.VITE_APP_API_URL
        }/api/pdf/${pdfId}?organizationId=${organizationId}&topicId=${topicId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${jwt}`,
  ...
  const handleIngesting = async () => {
    ...
    setLoading(true);
    await fetch(
      `${import.meta.env.VITE_APP_API_URL}/api/pdf/${
        activeFile?.jobId
      }/embed?organizationId=${organizationId}&topicId=${topicId}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
